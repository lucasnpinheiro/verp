...
class MyFormHelper extends BootstrapFormHelper {
    ...
    public function fileUpload($fieldName, array $options = array()) {
        ...
        if (isset($options['path']) AND ! empty($options['path'])) {
            ...
            //debug($list);
            $image = '<span><img class="img-lg img-border img-thumbnail" src="data:image/' . $list['extension'] . ';base64,' . base64_encode(file_get_contents($list['dirname'] . '/' . $list['basename'])) . '"></span>';
            unset($options['path']);
        }
        $id = $this->_domId($fieldName);
